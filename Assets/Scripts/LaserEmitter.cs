...
public class LaserEmitter : MonoBehaviour
{
    ...
    public LineRenderer coreLR; // URP/Unlit Additive + beam_core.png
    public LineRenderer glowLR; // URP/Unlit Additive + beam_glow.png

    [Header("Ring (Material Asset)")]
    public Material ringMaterial; // Should use URP/Lit as above

    [Header("Look")]
    public float coreWidth = 0.02f;
    public float glowWidthMultiplier = 3f;

    [Tooltip("Beam intensity (HDR). Keep modest to avoid ACES clipping.")]
    [Range(0.5f, 3.0f)] public float coreIntensity = 1.8f;
    [Range(0.5f, 3.0f)] public float glowIntensity = 2.0f;

    [Tooltip("Ring emission intensity (Lit/Emission). Keep modest so it stays colored.")]
    ...
    void OnEnable() { Ensure(); UpdateAll(true); }
    void OnValidate() { Ensure(); UpdateAll(true); }
    ...
    void UpdateAll(bool force)
    {
        ...
        // ACES-friendly primaries (deeper reds/greens/blues hold hue better)
        Color baseCol = laser switch
        {
            LaserType.Green => new Color(0.00f, 0.90f, 0.15f, 1f),
            LaserType.Blue => new Color(0.10f, 0.30f, 1.00f, 1f),
            _ => new Color(1.00f, 0.05f, 0.02f, 1f), // red without yellow skew
        };

        // Beam colors: keep core slightly colored (not pure white), glow fully colored
        ...
        SetRingMaterialEmission(baseCol * ringEmission);
    }
    ...
    // URP/Unlit: drive ONLY _BaseColor; no emission on the beam materials
    ...
    // URP/Lit: use Emission to keep hue under ACES; keep Base dark
    void SetRingMaterialEmission(Color emissionColor)
    {
        if (!ringMaterial) return;

        if (ringMaterial.HasProperty("_BaseColor"))
            ringMaterial.SetColor("_BaseColor", new Color(0.02f, 0.02f, 0.02f, 1f));

        if (ringMaterial.HasProperty("_EmissionColor"))
        {
            ringMaterial.EnableKeyword("_EMISSION");
            ringMaterial.SetColor("_EmissionColor", emissionColor);
        }
    }
}
